...
class MLLoggerWrapper:
    def __init__(self, mllogger=None, default_stack_offset=3, **default_log_kwargs):
        """
        Args:
    ...
    def log_run_stop(self, status, sync=True, unique=True, unique_log_rank=0, **kwargs):
        ...
        self.end(
            ...
            unique=unique,
            unique_log_rank=unique_log_rank,
            sync=sync,
            metadata=dict(status=status, **kwargs),
            internal_call=True,
        )
    ...
    def mlperf_submission_log(self, benchmark, num_nodes=None, org=None, platform=None):
        """Helper for logging submission entry."""
        if num_nodes is None:
            num_nodes = os.environ.get("SLURM_JOB_NUM_NODES", 1)

        if org is None:
            org = os.environ.get("MLPERF_SUBMISSION_ORG", "SUBMISSION_ORG_PLACEHOLDER")

        if platform is None:
            platform = os.environ.get(
                "MLPERF_SUBMISSION_PLATFORM", "SUBMISSION_PLATFORM_PLACEHOLDER"
            )

        self.event(
            key=constants.SUBMISSION_BENCHMARK, value=benchmark, internal_call=True
        )

        self.event(key=constants.SUBMISSION_ORG, value=org, internal_call=True)
        ...
        self.event(
            key=constants.SUBMISSION_STATUS, value=constants.ONPREM, internal_call=True
        )
